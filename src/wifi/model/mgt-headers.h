...
using AuthRequestElems = std::tuple<std::optional<WEPElems>,
                                     std::optional<ExtendedSupportedRatesIE>,
                                     std::optional<HtCapabilities>>;  
                                     
using AuthResponseElems = std::tuple<std::optional<ChallengeText>,
                                     std::optional<ExtendedSupportedRatesIE>,
                                     std::optional<HtCapabilities>>;                                    
...
/**
 * \ingroup wifi
 * Implement the header for management frames of type Auth request.
 */
class MgtAuthRequestHeader
    : public MgtHeaderInPerStaProfile<MgtAuthRequestHeader, AuthRequestElems>
{
    friend class WifiMgtHeader<MgtAuthRequestHeader, AuthRequestElems>;
    friend class MgtHeaderInPerStaProfile<MgtAuthRequestHeader, AuthRequestElems>;

public:
    ~MgtAuthRequestHeader() override = default;
     ...
     * \param algorithm the authentication algorithm (e.g., Open System or Shared Key).
     */
    void SetAuthAlgorithm(uint16_t algorithm);

    /**
     * Get the authentication algorithm.
     * \return the authentication algorithm.
     */
    uint16_t GetAuthAlgorithm() const;

    /**
     * Set the transaction sequence number.
     * \param seqNumber the transaction sequence number.
     */
    void SetTransactionSequenceNumber(uint16_t seqNumber);

    /**
     * Get the transaction sequence number.
     * \return the transaction sequence number.
     */
    uint16_t GetTransactionSequenceNumber() const;
...
private:
    uint16_t m_authAlgorithm{0};                 //!< Authentication algorithm
    uint16_t m_transactionSequenceNumber{0};    //!< Transaction sequence number
};

class MgtAuthResponseHeader
    : public MgtHeaderInPerStaProfile<MgtAuthResponseHeader, AuthResponseElems>
{
    friend class WifiMgtHeader<MgtAuthResponseHeader, AuthResponseElems>;
    friend class MgtHeaderInPerStaProfile<MgtAuthResponseHeader, AuthResponseElems>;

  public:
    ~MgtAuthResponseHeader() override = default;
     ...
     * \return the authentication algorithm number
     */
    uint16_t GetAuthenticationAlgorithmNumber() const;

    /**
     * Set the authentication algorithm number.
     *
     * \param algorithm the authentication algorithm number
     */
    void SetAuthenticationAlgorithmNumber(uint16_t algorithm);
     ...
     */
    void SetTransactionSequenceNumber(uint16_t seqNum);
  ...
  private:
    StatusCode m_code;                  //!< Status code
    uint16_t m_authAlgorithm{0};       //!< Authentication algorithm number
    uint16_t m_transactionSequenceNumber{0}; //!< Transaction sequence number
};
