    ...
    static TypeId tid =
        TypeId("ns3::ApWifiMac")
            ...
            .AddAttribute("SharedKeyAuth",
                            ...
                            StringValue(""),
                            MakeStringAccessor(&ApWifiMac::SetWEPKey, &ApWifiMac::GetWEPKey),
                            MakeStringChecker())
            .AddAttribute(
                "CwMinsForSta",
...
void
ApWifiMac::SetWEPKey(std::string key)
{
    m_wepkey = key;
}

std::string 
ApWifiMac::GetWEPKey() const
{
    return m_wepkey;
}
    ...
    else if (hdr->IsMgt())
    {
        ...
        else if (hdr->GetAddr1() == GetFrameExchangeManager(linkId)->GetAddress())
        {
            ...
            case WIFI_MAC_MGT_AUTHENTICATION:
                HandleAuthenticationRequest(packet, hdr->GetAddr2(), linkId);
                return;
...
void
ApWifiMac::HandleAuthenticationRequest(Ptr<const Packet> packet, Mac48Address clientAddress, uint8_t linkId)
{
    // NS_LOG_FUNCTION(this << clientAddress);
    std::cout<<"Auth Packet received at AP"<<std::endl;
    // Parse the Authentication Request frame

    MgtAuthRequestHeader authRequestHeader;
    packet->PeekHeader(authRequestHeader);
    
    uint16_t authAlgorithm = authRequestHeader.GetAuthAlgorithm();
    uint8_t seqNumber = authRequestHeader.GetTransactionSequenceNumber();
    
    NS_LOG_DEBUG("Received Authentication Request from " << clientAddress 
                 << " Algorithm: " << authAlgorithm 
                 << " Seq: " << (uint16_t)seqNumber);

    // // Check if the Authentication Algorithm is supported (Open System)
    // if (authAlgorithm != 0)
    // {
    //     // NS_LOG_DEBUG("Unsupported Authentication Algorithm. Sending failure.");
    //     StatusCode statusCode;
    //     statusCode.SetSuccess();
    //     SendAuthenticationResponse(clientAddress, statusCode, seqNumber + 1, linkId);
    //     return;
    // }

    if(seqNumber == 3 && authAlgorithm == 1){

        auto& elems = authRequestHeader.Get<WEPElems>();
        WEPElems wep_elems = elems.value();

        std::cout<<"Decrypting challenege text"<<std::endl;
        if(wep_elems.AuthenticateClient("Hello this is the text", m_wepkey)){
            StatusCode statusCode;
            statusCode.SetSuccess();
            std::cout<<"Client is authenticated"<<std::endl;
            SendAuthenticationResponse(authAlgorithm, clientAddress, statusCode, seqNumber, linkId);
        }
        else{
            StatusCode statusCode;
        ...
        // Send a successful Authentication Response
        StatusCode statusCode;
        statusCode.SetSuccess();
...
void ApWifiMac::SendAuthenticationResponse(uint16_t authAlgorithm, Mac48Address clientAddress, StatusCode statusCode, uint8_t seqNumber, u_int8_t linkId)
{
    // NS_LOG_FUNCTION(this << clientAddress << statusCode << (uint16_t)seqNumber);
    // Create the Authentication Response header
    std::cout<<"Sending auth response"<<std::endl;
    MgtAuthResponseHeader authResponseHeader;
    authResponseHeader.SetAuthenticationAlgorithmNumber(authAlgorithm); // Open System
    authResponseHeader.SetTransactionSequenceNumber(seqNumber + 1);
    authResponseHeader.SetStatusCode(statusCode);

    if(authAlgorithm == 1 && seqNumber == 1){
        std::cout<<"Setting challenge text"<<std::endl;
        std::string text = "Hello this is the text";
        ChallengeText ch_text(text);

        authResponseHeader.Get<ChallengeText>() = ch_text;
    }

    // Create the packet and add the header
    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(authResponseHeader);

    WifiMacHeader hdr;
    hdr.SetType(WIFI_MAC_MGT_AUTHENTICATION);
    hdr.SetAddr1(clientAddress); // Destination: Client
    hdr.SetAddr2(GetLink(linkId).feManager->GetAddress());
    hdr.SetAddr3(GetLink(linkId).feManager->GetAddress());
    ...
    NS_LOG_DEBUG("Sent Authentication Response to " << clientAddress);
}
