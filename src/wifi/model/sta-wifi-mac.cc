...
StaWifiMac::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::StaWifiMac")
            ...
            .AddAttribute("AuthRequestTimeout",
                          "The interval between two consecutive auth request attempts.",
                          TimeValue(Seconds(0.5)),
                          MakeTimeAccessor(&StaWifiMac::m_authRequestTimeout),
                          MakeTimeChecker())
            .AddAttribute("OpenAuth",
                            "Enable or disable Open Auth",
                            BooleanValue(false),
                            MakeBooleanAccessor(&StaWifiMac::SetOpenAuth, &StaWifiMac::GetOpenAuth),
                            MakeBooleanChecker())
            .AddAttribute("SharedKeyAuth",
                            "Enable or disable SharedKeyAuth",
                            StringValue(""),
                            MakeStringAccessor(&StaWifiMac::SetWEPKey, &StaWifiMac::GetWEPKey),
                            MakeStringChecker())
            .AddAttribute(
                "ActiveProbing",
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Do inintialize"<<std::endl;
...
StaWifiMac::AssignStreams(int64_t stream)
{
    ...
    std::cout<<"Assign stream"<<std::endl;
...
StaWifiMac::SetAssocManager(Ptr<WifiAssocManager> assocManager)
{
    ...
    std::cout<<"Set assoc manager"<<std::endl;
...
StaWifiMac::SetEmlsrManager(Ptr<EmlsrManager> emlsrManager)
{
    ...
    std::cout<<"Set EMLS manager"<<std::endl;
...
StaWifiMac::SetActiveProbing(bool enable)
{
    ...
    std::cout<<"Set Active probing"<<std::endl;
...
void
StaWifiMac::SetOpenAuth(bool enable)
{
    NS_LOG_FUNCTION(this << enable);
    std::cout<<"Set Active probing"<<std::endl;
    m_openauth = enable;
...
bool
StaWifiMac::GetOpenAuth() const
{
    return m_openauth;
}

void
StaWifiMac::SetWEPKey(std::string key)
{
    m_wepkey = key;
}

std::string 
StaWifiMac::GetWEPKey() const
{
    return m_wepkey;
}
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Set wifi phys"<<std::endl;
...
{
    std::cout<<"Notify emls mode changed"<<std::endl;
    std::stringstream ss;
...
StaWifiMac::SendProbeRequest(uint8_t linkId)
{
    ...
    std::cout<<"Send probe request"<<std::endl;
    WifiMacHeader hdr;
...
StaWifiMac::GetAssociationRequest(bool isReassoc, uint8_t linkId) const
{
    ...
    std::cout<<"Get assoc request"<<std::endl;
...
{
    NS_LOG_FUNCTION(this << isReassoc << +linkId);
    std::cout<<"Get multilink element"<<std::endl;
...
StaWifiMac::GetTidToLinkMappingElements(WifiTidToLinkMappingNegSupport apNegSupport)
{
    ...
    std::cout<<"Get id link mapping elements"<<std::endl;
...
void
StaWifiMac::SendAuthenticationRequest(uint16_t authAlgorithm, uint16_t transactionSeqNum, std::string challengeText="")
{
    std::cout<<"Send Auth Request"<<std::endl;
    auto it = GetLinks().cbegin();
    uint8_t linkId = it->first;
    auto& link = GetLink(linkId);
    NS_ABORT_MSG_IF(!link.bssid.has_value(),
                    "No BSSID set for the link on which the Auth Request is to be sent");
    WifiMacHeader hdr;
    hdr.SetType(WIFI_MAC_MGT_AUTHENTICATION);
    ...
    MgtAuthRequestHeader authBody;
    authBody.SetAuthAlgorithm(authAlgorithm);         // Open System or Shared Key
    authBody.SetTransactionSequenceNumber(transactionSeqNum); // Sequence number

    if(challengeText != "" && authAlgorithm==1 && transactionSeqNum == 3){
        std::cout<<"Encrypting challenge text and sending it"<<std::endl;
        authBody.Get<WEPElems>() = WEPElems(challengeText, m_wepkey);
    }
    
    packet->AddHeader(authBody);
    
    // std::cout << "Packet size after adding headers: " << packet->GetSize() << std::endl;

    // uint32_t bufferSize = packet->GetSize();

    // // Create a vector to hold the packet data
    // std::vector<uint8_t> buffer(bufferSize);

    // // Copy packet data into the buffer
    // packet->CopyData(buffer.data(), bufferSize);
    ...
    m_authRequestEvent =
        Simulator::Schedule(m_authRequestTimeout, &StaWifiMac::AuthRequestTimeout, this);
}
...
StaWifiMac::SendAssociationRequest(bool isReassoc)
{
    ...
    std::cout<<"Send Association request"<<std::endl;
    auto it = GetLinks().cbegin();
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Try to ensure associated"<<std::endl;
    switch (m_state)
    {
    case ASSOCIATED:
        return;
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Scanning timeout"<<std::endl;
    if (!bestAp.has_value())
    {
    ...
    if(GetOpenAuth()){
        ...
        SendAuthenticationRequest(0,1);
    }
    else if(GetWEPKey() != ""){
        SetState(WAIT_AUTH_RESP);
        std::cout<<"Sending shared key auth request"<<std::endl;
        SendAuthenticationRequest(1,1);
    }
    else{
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Assoc request timeout"<<std::endl;
    SetState(WAIT_ASSOC_RESP);
    SendAssociationRequest(false);
}

void
StaWifiMac::AuthRequestTimeout()
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Auth request timeout"<<std::endl;
    SetState(WAIT_AUTH_RESP);
    SendAuthenticationRequest(0, 1);
}
...
{
    NS_LOG_FUNCTION(this);
    std::cout<<"Missed Beacons"<<std::endl;
...
StaWifiMac::Disassociated()
{
    ...
    std::cout<<"Disassociated"<<std::endl;
    for (const auto& [id, link] : GetLinks())
    {
...
StaWifiMac::RestartBeaconWatchdog(Time delay)
{
    ...
    std::cout<<"Restart Beacon watch dog"<<std::endl;
...
bool
StaWifiMac::IsWaitAuthResp() const
{
    return m_state == WAIT_AUTH_RESP;
}
...
StaWifiMac::BlockTxOnLink(uint8_t linkId, WifiQueueBlockedReason reason)
{
    ...
    std::cout<<"Block Tx on link"<<std::endl;
...
{
    std::cout<<"unblockTx on link"<<std::endl;
    std::stringstream ss;
    std::copy(linkIds.cbegin(), linkIds.cend(), std::ostream_iterator<uint16_t>(ss, " "));
...
{
    NS_LOG_FUNCTION(this << *mpdu << +linkId);
    std::cout<<"Receive Probe Response"<<std::endl;
    const WifiMacHeader& hdr = mpdu->GetHeader();
...
{
    NS_LOG_FUNCTION(this << *mpdu << +linkId);
    std::cout<<"Receive Assoc Response"<<std::endl;
    const WifiMacHeader& hdr = mpdu->GetHeader();
...
void 
StaWifiMac::ReceiveAuthResp(Ptr<const WifiMpdu> mpdu, uint8_t linkId) 
{
    NS_LOG_FUNCTION(this << *mpdu << +linkId);
    std::cout<<"Receive Auth Response"<<std::endl;
    const WifiMacHeader& hdr = mpdu->GetHeader();
    ...
    MgtAuthResponseHeader authResp;
    mpdu->GetPacket()->PeekHeader(authResp);
    

    if (m_authRequestEvent.IsPending())
    {
    ...
    {
    	if(authResp.GetTransactionSequenceNumber() == 2 && authResp.GetAuthenticationAlgorithmNumber() == 1)
    	{
    	    if(authResp.Get<ChallengeText>()){
                auto& challenge_text = authResp.Get<ChallengeText>();
                ChallengeText text = challenge_text.value();
                std::cout<<text.PeekString()<<std::endl;

                SendAuthenticationRequest(1, 3, text.PeekString());
            }
    	}

    	if(authResp.GetTransactionSequenceNumber() == 2 && authResp.GetAuthenticationAlgorithmNumber() == 0)
    	{
    	    SetState(WAIT_ASSOC_RESP);
            SendAssociationRequest(false);
    	}

        if(authResp.GetTransactionSequenceNumber() == 4 && authResp.GetAuthenticationAlgorithmNumber() == 1)
    	{
    	    SetState(WAIT_ASSOC_RESP);
            SendAssociationRequest(false);
    	}
        ...
        NS_ASSERT_MSG(authResp.GetStatusCode().IsSuccess(),"Wrong Passphrase");
    }
}
...
{
    NS_LOG_FUNCTION(this << linkId);
    std::cout<<"SetPmModeAfterAssociation"<<std::endl;
...
{
    NS_LOG_FUNCTION(this << +linkId);
    std::cout<<"CheckSupportedRates"<<std::endl;
                         ...
                         uint8_t linkId)
{
    ...
    std::cout<<"UpdateApInfo"<<std::endl;
...
StaWifiMac::SetPowerSaveMode(const std::pair<bool, uint8_t>& enableLinkIdPair)
{
    ...
    std::cout<<"SetPowerSaveMode"<<std::endl;
    auto& link = GetLink(linkId);
