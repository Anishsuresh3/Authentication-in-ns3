...
class StaWifiMac : public WifiMac
{
  ...
  private:
     ...
     *
     * \param enable enable or disable Open Auth.
     */
    void SetOpenAuth(bool enable);
    /**
     * Return whether Open Auth is enabled.
     *
     * \return true if Open Auth is enabled, false otherwise
     */
    bool GetOpenAuth() const;

    std::string GetWEPKey() const;

    void SetWEPKey(std::string key);
    ...
    /**
     * Process the Auth Response frame received on the given link.
     *
     * \param mpdu the MPDU containing the Auth Response frame
     * \param linkId the ID of the given link
     */
    void ReceiveAuthResp(Ptr<const WifiMpdu> mpdu, uint8_t linkId);
     ...
     * Send Auth Request
     */
    void SendAuthenticationRequest(uint16_t authAlgorithm, uint16_t transactionSeqNum, std::string challengeText);
    /**
    ...
    /**
     * This method is called after the auth timeout occurred.
     */
    void AuthRequestTimeout();
    /**
    ...
    /**
     * Return whether we are waiting for an auth response from an AP.
     *
     * \return true if we are waiting for an auth response from an AP, false otherwise
     */
    bool IsWaitAuthResp() const;
    /**
